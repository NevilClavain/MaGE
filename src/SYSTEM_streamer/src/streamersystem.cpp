...
void StreamerSystem::run()
{
    ...
    while (!m_newly_added_entities.empty())
    {
        ...
        if (newly_added_entity->hasAspect(core::worldAspect::id))
        {
            ...
            bool computed = compute_entity(newly_added_entity, world_aspect);
            if (computed)
            {
...
void StreamerSystem::parseCombiner(StreamerSystem::Combiners p_combiner, const std::string& p_jsonsource)
{
    ...
    m_combiners[p_combiner] = combiner;
}
...
RendergraphBlueprint StreamerSystem::diffuseRenderGraphBluePrint()
{
    ...
    bp.mininmal_channel_type = "diffuse";
    return bp;
}
...
{ 
    ...
    const auto combiner{ m_combiners.at(p_combiner) };
...
void StreamerSystem::buildRendergraphPart(const RendergraphBlueprint& p_blueprint, const std::string& p_parentEntityId, int p_w_width, int p_w_height, float p_characteristics_v_width, float p_characteristics_v_height)
{
